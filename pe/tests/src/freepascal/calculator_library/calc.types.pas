...
type
  ...
  TResultVariant = record
    case Operation: TArithmeticOperation of
      opAdd, opSubtract, opMultiply: (A, B, IntResult: integer);
      opDivide, opMod: (DivRemResult: TDivRemResult);
  end;
...
begin
  case FResult.Operation of
    opAdd, opSubtract, opMultiply: Result := FResult.IntResult;
    opDivide: Result := FResult.DivRemResult.Divisor;
    opMod: Result := FResult.DivRemResult.Remainder;
    else
      Result := $7FFFFFFF;
  end;
end;
